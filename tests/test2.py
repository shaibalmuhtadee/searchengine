...
class TestCrawlerOutputCase2(unittest.TestCase):
    def setUp(self):
        self.crawler = crawler(None, "urls.txt")
        self.crawler._url_queue = [("https://en.wikipedia.org/wiki/Python_(programming_language)", 0)]
    ...
    def test_crawl_output(self):
        ...
        with open("tests/output_test2.txt", "r") as f:
            expected_output = f.read()

        # Compare actual output to expected output
        self.assertEqual(actual_output, expected_output, "Crawler output does not match expected output for Test Case 2.")
